...
EOF
  ...
  echo "deb ${DEFAULT_MIRROR} ${BRANCH} main non-free contrib" > "${rootfsDir}"/etc/apt/sources.list
  ...
  vrun tar --ignore-failed-read -C "${rootfsDir}"/ -czf "${OUTDIR}/${OUTPUT}.tar.gz" ./
...
EOF
find "${OUTDIR}/" -maxdepth 1 -type f -name "${OUTPUT}*" | sed 's_^_* _'
